...
def binary():
    return 'CGO_ENABLED=0 GOOS=linux GOARCH=amd64 GO111MODULE=on go build -o bin/manager cmd/main.go'
...
local_resource('recompile', generate() + binary(), deps=['controllers', 'cmd/main.go'])
